...
const server = new Server(
  { name: "ankjyotish-seo-mcp-sse", version: "1.2.0" },
  { capabilities: { tools: {} } }
);
...
async function logMcpAudit(action, details) {
  try {
    await supabase.from("admin_audit_log").insert({
      action: `MCP_SSE_${action.toUpperCase()}`,
      details,
...
// 3. Setup SSE Transports & Express Endpoints
let transports = {};
...
app.get("/health", (req, res) => {
  res.json({ status: "online", mcp_server: "AnkJyotish SEO MCP SSE", port: PORT });
});

app.listen(PORT, () => {
  console.log(`🌐 AnkJyotish MCP SSE Server running on http://localhost:${PORT}/sse`);
});
